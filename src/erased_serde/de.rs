...
pub trait Visitor<'de> {
    fn erased_expecting(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result;
...
mod tests {
        ...
        T: serde::Deserialize<'de> + PartialEq + Debug,
    {
        ...
        // test borrowed trait object
        {
            ...
            let de: &mut dyn Deserializer<'_> = &mut Deserializer::erase(&mut de);
        ...
        {
            let mut de = serde_json::Deserializer::from_slice(json);
            let mut de: Box<dyn Deserializer<'_>> = Box::new(Deserializer::erase(&mut de));
    ...
    fn assert_deserializer() {
        ...
        assert::<&mut dyn Deserializer<'_>>();
        assert::<&mut (dyn Deserializer<'_> + Send)>();
        assert::<&mut (dyn Deserializer<'_> + Sync)>();
        assert::<&mut (dyn Deserializer<'_> + Send + Sync)>();
        assert::<&mut (dyn Deserializer<'_> + Sync + Send)>();

        assert::<Box<dyn Deserializer<'_>>>();
        assert::<Box<dyn Deserializer<'_> + Send>>();
        assert::<Box<dyn Deserializer<'_> + Sync>>();
        assert::<Box<dyn Deserializer<'_> + Send + Sync>>();
        assert::<Box<dyn Deserializer<'_> + Sync + Send>>();
    }
}
