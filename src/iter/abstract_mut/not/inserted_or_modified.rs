...
impl<'tmp, T: Component> AbstractMut for Not<InsertedOrModified<&'tmp SparseSet<T, T::Tracking>>> {
    type Out = &'tmp T;
    type Index = usize;
...
impl<'tmp, T: Component> AbstractMut for Not<InsertedOrModified<FullRawWindowMut<'tmp, T>>> {
    type Out = Mut<'tmp, T>;
    type Index = usize;
