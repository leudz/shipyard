...
use super::{AbstractMut, IntoAbstract, ParFilterWithId1, ParTightFilter1, ParUpdateFilter1};
#[cfg(feature = "parallel")]
...
{
    Tight(ParTightFilter1<T, P>),
    Update(ParUpdateFilter1<T, P>),
}

#[cfg(feature = "parallel")]
impl<T: IntoAbstract, P> ParFilter1<T, P>
where
    <T::AbsView as AbstractMut>::Out: Send,
{
    pub fn with_id(self) -> ParFilterWithId1<T, P> {
        match self {
            ParFilter1::Tight(iter) => ParFilterWithId1::Tight(iter.with_id()),
            ParFilter1::Update(iter) => ParFilterWithId1::Update(iter.with_id()),
        }
    }
}
