...
impl<T: Send + Sync + Component> BulkInsert for T {
    fn bulk_insert<I: IntoIterator<Item = Self>>(
        ...
        sparse_set.unclassified_bucket.data.extend(iter);

        // generate new EntityId for the entities created
        let entities_len = entities.data.len();
        let old_len = sparse_set.unclassified_bucket.dense.len();
        let new_entities_count = sparse_set.unclassified_bucket.data.len() - old_len;
        ...
        sparse_set
            .unclassified_bucket
            .dense
            .extend_from_slice(new_entities);
        ...
        impl<$type1: Send + Sync + Component, $($type: Send + Sync + Component,)*> BulkInsert for ($type1, $($type,)*) {
            ...
            fn bulk_insert<Source: IntoIterator<Item = Self>>(all_storages: &mut AllStorages, iter: Source) -> BulkEntityIter<'_> {
                ...
                for ($type1, $($type,)*) in iter {
                    $sparse_set1.unclassified_bucket.data.push($type1);
                    $(
                        $sparse_set.unclassified_bucket.data.push($type);
                    )*
                }

                let entities_len = entities.data.len();
                let new_entities_count = $sparse_set1.unclassified_bucket.data.len() - $sparse_set1.unclassified_bucket.dense.len();
                let new_entities = entities.bulk_generate(new_entities_count);

                $sparse_set1.unclassified_bucket.dense.extend_from_slice(new_entities);
                $(
                    $sparse_set.unclassified_bucket.dense.extend_from_slice(new_entities);
                )*

                if $sparse_set1.is_tracking_insertion() {
                    $sparse_set1.unclassified_bucket.insertion_data.extend(new_entities.iter().map(|_| TrackingTimestamp::new(0)));
                }
                if $sparse_set1.is_tracking_modification() {
                    $sparse_set1.unclassified_bucket.modification_data.extend(new_entities.iter().map(|_| TrackingTimestamp::new(0)));
                }
                $(
                    if $sparse_set.is_tracking_insertion() {
                        $sparse_set.unclassified_bucket.insertion_data.extend(new_entities.iter().map(|_| TrackingTimestamp::new(0)));
                    }
                    if $sparse_set.is_tracking_modification() {
                        $sparse_set.unclassified_bucket.modification_data.extend(new_entities.iter().map(|_| TrackingTimestamp::new(0)));
                    }
                )*
