...
impl<'w, T: Component> FullRawWindow<'w, T> {
    ...
    pub(crate) fn from_view<Track: Tracking>(view: &View<'_, T, Track>) -> Self {
        ...
        FullRawWindow {
            sparse,
            sparse_len,
            dense: NonNull::new(view.dense[0].as_ptr().cast_mut()).unwrap(),
            dense_len: view.dense[0].len(),
            data: view.data[0].as_ptr(),
            insertion_data: view.insertion_data[0].as_ptr(),
            modification_data: view.modification_data[0].as_ptr(),
    ...
    pub(crate) fn from_owned_view<Track: Tracking>(
        ...
        (
            FullRawWindow {
                sparse,
                sparse_len,
                dense: NonNull::new(sparse_set.dense[0].as_ptr().cast_mut()).unwrap(),
                dense_len: sparse_set.dense[0].len(),
                data: sparse_set.data[0].as_ptr(),
                insertion_data: sparse_set.insertion_data[0].as_ptr(),
                modification_data: sparse_set.modification_data[0].as_ptr(),
                last_insertion,
                last_modification,
                current,
                _phantom: PhantomData,
...
impl<'w, T: Component, Track> FullRawWindowMut<'w, T, Track> {
    ...
    pub(crate) fn new(view: &mut ViewMut<'_, T, Track>) -> Self {
        ...
        FullRawWindowMut {
            sparse,
            sparse_len,
            dense: NonNull::new(view.dense[0].as_mut_ptr().cast()).unwrap(),
            dense_len: view.dense[0].len(),
            data: view.data[0].as_mut_ptr(),
            insertion_data: view.insertion_data[0].as_ptr(),
            modification_data: view.modification_data[0].as_mut_ptr(),
            last_insertion: view.last_insertion,
            last_modification: view.last_modification,
            current: view.current,
    ...
    pub(crate) fn new_owned<TRACK>(
        ...
        (
            FullRawWindowMut {
                sparse,
                sparse_len,
                dense: NonNull::new(sparse_set.dense[0].as_mut_ptr().cast()).unwrap(),
                dense_len: sparse_set.dense[0].len(),
                data: sparse_set.data[0].as_mut_ptr(),
                insertion_data: sparse_set.insertion_data[0].as_ptr(),
                modification_data: sparse_set.modification_data[0].as_mut_ptr(),
                last_insertion,
                last_modification,
                current,
