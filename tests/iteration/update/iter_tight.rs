...
fn basic() {
    ...
    assert_eq!(u32s.inserted().try_fast_iter().unwrap().count(), 3);
    assert_eq!(u32s.modified().try_fast_iter().unwrap().count(), 0);
    assert_eq!(
        u32s.inserted_or_modified().try_fast_iter().unwrap().count(),
        3
    );
    ...
    if let iter::Iter::Tight(mut iter) = (&u32s, &i16s).iter() {
    ...
    if let iter::Iter::Tight(mut iter) = (&mut u32s, &mut i16s).iter() {
        assert_eq!(iter.size_hint(), (3, Some(3)));
        let mut next = iter.next().unwrap();
        *next.0 += 1;
        *next.0 -= 1;
    ...
    if let iter::Iter::Tight(mut iter) = (&i16s, &u32s).iter() {
        assert_eq!(iter.size_hint(), (3, Some(3)));
    ...
    if let iter::Iter::Tight(mut iter) = (&mut i16s, &mut u32s).iter() {
        assert_eq!(iter.size_hint(), (3, Some(3)));
        let mut next = iter.next().unwrap();
        *next.1 += 1;
        *next.1 -= 1;
