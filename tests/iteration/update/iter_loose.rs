...
fn basic() {
    ...
    if let iter::Iter::Loose(mut iter) = (&u32s, &i16s).iter() {
    ...
    if let iter::Iter::Loose(mut iter) = (&mut u32s, &mut i16s).iter() {
        assert_eq!(iter.size_hint(), (3, Some(3)));
        let mut next = iter.next().unwrap();
        *next.0 += 1;
        *next.0 -= 1;
    ...
    if let iter::Iter::Loose(mut iter) = (&i16s, &u32s).iter() {
        assert_eq!(iter.size_hint(), (3, Some(3)));
    ...
    if let iter::Iter::Loose(mut iter) = (&mut i16s, &mut u32s).iter() {
        assert_eq!(iter.size_hint(), (3, Some(3)));
        let mut next = iter.next().unwrap();
        *next.1 += 1;
        *next.1 -= 1;
