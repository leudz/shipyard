...
fn basic() {
    ...
    if let iter::FastIter::Tight(mut iter) = (&u32s).try_fast_iter().unwrap() {
    ...
    if let iter::FastIter::Tight(mut iter) = (&mut u32s).try_fast_iter().unwrap() {
        assert_eq!(iter.size_hint(), (4, Some(4)));
        assert_eq!(*iter.next().unwrap(), 0);
        assert_eq!(*iter.next().unwrap(), 2);
        assert_eq!(*iter.next().unwrap(), 4);
    ...
    if let iter::FastIter::Tight(mut iter) = (&i16s).try_fast_iter().unwrap() {
        assert_eq!(iter.size_hint(), (4, Some(4)));
    ...
    if let iter::FastIter::Tight(mut iter) = (&mut i16s).try_fast_iter().unwrap() {
        assert_eq!(iter.size_hint(), (4, Some(4)));
    ...
    if let iter::FastIter::Tight(mut iter) = (&u32s, &i16s).try_fast_iter().unwrap() {
    ...
    if let iter::FastIter::Tight(mut iter) = (&mut u32s, &mut i16s).try_fast_iter().unwrap() {
        assert_eq!(iter.size_hint(), (3, Some(3)));
    ...
    if let iter::FastIter::Tight(mut iter) = (&i16s, &u32s).try_fast_iter().unwrap() {
        assert_eq!(iter.size_hint(), (3, Some(3)));
    ...
    if let iter::FastIter::Tight(mut iter) = (&mut i16s, &mut u32s).try_fast_iter().unwrap() {
        assert_eq!(iter.size_hint(), (3, Some(3)));
...
fn with_id() {
    ...
    let mut iter = (&u32s).try_fast_iter().unwrap().with_id();
    ...
    assert!(iter.next().is_none());
    let mut iter = (&mut u32s).try_fast_iter().unwrap().with_id();
    ...
    let mut iter = i16s.try_fast_iter().unwrap().with_id();
    ...
    assert!(iter.next().is_none());
    let mut iter = (&mut i16s).try_fast_iter().unwrap().with_id();
    ...
    let mut iter = (&u32s, &i16s).try_fast_iter().unwrap().with_id();
    assert_eq!(iter.next().unwrap(), (id0, (&0, &10)));
    assert_eq!(iter.next().unwrap(), (id2, (&2, &12)));
    assert_eq!(iter.next().unwrap(), (id4, (&4, &14)));
    assert!(iter.next().is_none());
    let mut iter = (&mut u32s, &mut i16s).try_fast_iter().unwrap().with_id();
    assert_eq!(
    ...
    let mut iter = (&i16s, &u32s).try_fast_iter().unwrap().with_id();
    assert_eq!(iter.next().unwrap(), (id0, (&10, &0)));
    assert_eq!(iter.next().unwrap(), (id2, (&12, &2)));
    assert_eq!(iter.next().unwrap(), (id4, (&14, &4)));
    assert!(iter.next().is_none());
    let mut iter = (&mut i16s, &mut u32s).try_fast_iter().unwrap().with_id();
    assert_eq!(
        iter.next().map(|(id, (x, y))| (id, (*x, *y))).unwrap(),
        (id0, (10, 0))
    );
...
fn empty() {
    ...
    assert!(u32s.try_fast_iter().unwrap().next().is_none());
    assert!(u32s.try_fast_iter().unwrap().with_id().next().is_none());
    assert!(usizes.try_fast_iter().unwrap().next().is_none());
    assert!(usizes.try_fast_iter().unwrap().with_id().next().is_none());
    assert!((&usizes, &u32s).try_fast_iter().unwrap().next().is_none());
    assert!((&usizes, &u32s)
        .try_fast_iter()
        .unwrap()
        .with_id()
