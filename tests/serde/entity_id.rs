...
fn entity_id_serde() {
    ...
    let (entity_id0, _) = world.run(
        |(mut entities, mut u64s): (EntitiesViewMut, ViewMut<U64>)| {
            let entity_id0 = entities.add_entity(&mut u64s, U64(0));
            check_roundtrip(entity_id0, "{\"index\":0,\"gen\":0}");

            let entity_id1 = entities.add_entity(&mut u64s, U64(1));
    ...
    world.run(
        |(mut entities, mut u64s): (EntitiesViewMut, ViewMut<U64>)| {
            let entity_id2 = entities.add_entity(&mut u64s, U64(2));
            // generation was bumped
            check_roundtrip(entity_id2, "{\"index\":0,\"gen\":1}");

            let entity_id3 = entities.add_entity(&mut u64s, U64(1));
